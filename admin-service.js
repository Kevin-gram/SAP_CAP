...
module.exports = class AdminService extends cds.ApplicationService { init() {
  ...
  this.on('createAuthor', async (req) => {
    if (!req.user.is('Admin')) return req.error(403, 'You are not authorized to create authors');
  ...
  this.on('createBook', async (req) => {
    if (!req.user.is('Admin')) return req.error(403, 'You are not authorized to create books');
